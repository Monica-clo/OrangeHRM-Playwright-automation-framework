#!/usr/bin/env bash
# Usage: ./run_tests.sh [workflow|positive|negative|smoke|regression|perf] [extra pytest / k6 options]
#   ./run_tests.sh                    -> all 5 E2E workflows
...
case "${1:-}" in
  ...
  workflow|positive|negative|smoke|regression)
    marker="$1"; shift
